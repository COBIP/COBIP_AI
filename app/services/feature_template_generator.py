...
_QUALITY_LLM_FULL_GENERATION_MODE = "quality_llm_full"
...
class FeatureTemplateGenerator:
    ...
    def _generate_llm_full_first(
        ...
        *,
        ...
        try:
            ...
            deferred_sections = compute_deferred_sections(request, normalized_dict)
        except (RuntimeError, ValidationError, TypeError, ValueError) as exc:
            enhancement_ms = max(0, int((time.perf_counter() - t0) * 1000))
            logger.warning(
                "Feature template LLM full-first failed; trying instant fallback: "
                "featureName=%s, errorType=%s, errorSummary=%s",
                request.featureName,
                type(exc).__name__,
                self._summarize_generation_error(exc),
            )
            if settings.FEATURE_TEMPLATE_INSTANT_SKELETON_ENABLED:
                return self._generate_instant_fallback(
                    request,
                    applied_refs=applied_refs,
                    gen_meta=gen_meta,
        ...
        logger.info(
            "Feature template generated by LLM full-first: featureName=%s, provider=%s",
            request.featureName,
            self._llm_service.provider,
        )
        return self._build_generate_result(
            template=template,
            source=self._llm_service.provider,
            applied_refs=applied_refs,
            generation_mode=_QUALITY_LLM_FULL_GENERATION_MODE,
            gen_meta=gen_meta,
            skeleton_first=False,
            deferred_sections=deferred_sections,
            fallback_used=False,
            initial_llm_enhancement_attempted=True,
            initial_llm_enhancement_succeeded=True,
            initial_llm_enhancement_ms=enhancement_ms,
        )

    def _generate_instant_fallback(
        self,
        request: FeatureTemplateGenerateRequest,
        *,
        applied_refs: list[dict[str, Any]],
        ...
        try:
            ...
            logger.warning(
                "Instant skeleton fallback failed; using mock: featureName=%s, error=%s",
                request.featureName,
                self._summarize_generation_error(exc),
            )
            return self._generate_mock_fallback(
        ...
        logger.info(
            "Feature template instant skeleton fallback: featureName=%s, source=instant",
            request.featureName,
        )
        return self._build_generate_result(
            template=template,
            ...
            fallback_used=True,
            initial_llm_enhancement_attempted=True,
            initial_llm_enhancement_succeeded=False,
            initial_llm_enhancement_ms=initial_llm_enhancement_ms,
        )
    ...
    def deserialize_cache_entry(cached_ft: dict[str, Any]) -> FeatureTemplateGenerateResult:
        return FeatureTemplateGenerateResult(
            ...
            generationMode=str(cached_ft.get("generationMode") or "quality_llm_full"),  # type: ignore[arg-type]
            skeletonFirst=bool(cached_ft.get("skeletonFirst", False)),
            deferredSections=(
            ...
            fallbackUsed=bool(cached_ft.get("fallbackUsed", False)),
        )
    ...
    def _build_generate_result(
        *,
        ...
        skeleton_meta: dict[str, Any] | None = None,
        skeleton_first: bool = False,
        ...
        meta = gen_meta if gen_meta is not None else (skeleton_meta or {})
        return FeatureTemplateGenerateResult(
            template=template,
            source=source,  # type: ignore[arg-type]
            appliedReferences=applied_refs,
            generationMode=generation_mode,  # type: ignore[arg-type]
            skeletonFirst=skeleton_first,
            deferredSections=(
                list(_DEFERRED_INITIAL_SECTIONS)
                if deferred_sections is None
                else list(deferred_sections)
            ),
            fastSkeletonEnabled=bool(meta.get("fastSkeletonEnabled")),
            ultraFastSkeletonEnabled=bool(meta.get("ultraFastSkeletonEnabled")),
            skeletonMaxTokens=meta.get("skeletonMaxTokens"),
            skeletonRagTopK=meta.get("skeletonRagTopK"),
            skeletonRagContentMaxChars=meta.get("skeletonRagContentMaxChars"),
